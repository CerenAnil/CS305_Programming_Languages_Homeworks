...
(define built-in-op? (lambda (op-symbol)
	(or
      (equal? op-symbol '+)
      (equal? op-symbol '-)
      (equal? op-symbol '*)
...
(define f-list? (lambda (e)
	(and (list? e) (symbol? (car e)) (or (null? (cdr e)) (f-list? (cdr e))))))
  
(define define-stmt? (lambda (e)
	(and (list? e) (equal? (car e) 'define) (symbol? (cadr e)) (= (length e) 3))))

(define lambda-stmt? (lambda (e)
	(and (list? e) (equal? (car e) 'lambda) (f-list? (cadr e)) (not (define-stmt? (caddr e))))))

(define lambda-op? (lambda (e)
	(and (list? e) (equal? (car e) 'lambda) (formal-set? (cadr e)) (= (length e) 3) )))
...
(define repl (lambda (env)
  (let* (
         ...
         (new-env (if (define-stmt? expr)
                      (extend-env (cadr expr) (if (define-stmt? (caddr expr)) (caddr expr) (s8-interpret (caddr expr) env)) env) env))
         (val (if (define-stmt? expr)
                  (cadr expr)(s8-interpret expr env)))
     ...
     (define s8-interpret (lambda (e env)
       (cond
         ...
         ((if-stmt? e) (if (eq? (s8-interpret (cadr e) env) 0)
            ( s8-interpret (cadddr e) env)
                ( s8-interpret (caddr e) env)))
         ((let-stmt? e) (let ((names (map car (cadr e))) (inits (map cadr (cadr e))))
